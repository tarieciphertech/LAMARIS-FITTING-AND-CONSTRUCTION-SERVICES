  ...
  return (
    <div className="site-shell">
      ...
      <main>
        ...
        <section id="properties" className="section"><div className="container"><div className="section-heading"><div><span className="kicker">PROPERTY OPPORTUNITIES</span><h2>Featured Properties</h2></div><a href="#contact" className="text-link">Need something specific? <ChevronRight size={17} /></a></div><div className="property-grid">{filteredProperties.map((property) => <PropertyCard key={property.id} property={property} />)}</div>{!filteredProperties.length && <div className="empty-state">No listings match those filters yet. <a href={whatsappUrl()} target="_blank" rel="noreferrer">Ask Lamaris directly.</a></div>}</div></section>
